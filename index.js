...
module.exports = function (log, isReady, mapper) {
  ...
  var mapStream = opts => {
    if (opts.values === false) {} else if (opts.seqs === false) { return asyncMap(mapper) } else {
  ...
  var flume = {
    ...
    use: function (name, createView) {
      if (~Object.keys(flume).indexOf(name)) { throw new Error(name + ' is already in use!') }
      throwIfClosed('use')
    ...
    close: function (cb) {
      ...
      cont.para(map(flume.views, function (sv, k) {
        ...
        if (!log.close) { cb(err) } else { log.close(cb) }
      })
    },
    views: {}
  }
